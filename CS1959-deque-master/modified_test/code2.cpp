...
bool isEqual(Ans& ans, Test& test) {
    ...
    for (int i = 0; i < ans.size(); i++) {
        if (randnum() % 2) {
            if (ans[i] != test[i]) {
                std::cout << "line31: " << i << std::endl;
                return false;
            }
        } else {
            if (ans.at(i) != test.at(i)) {
                std::cout << "line36: " << i << std::endl;
                return false;
            }
        }
    }
    ...
    if (ans.empty() != test.empty() || ans.front() != test.front() ||
        ans.back() != test.back()) {
        std::cout << "why we were still here" << std::endl;
        return false;
    }
...
template <typename Ans, typename Test>
void randnumFill(Ans& ans, Test& test, int n = 2e4) {
    for (int i = 0; i < n; i++) {
        int x = randnum();
        if (randnum() % 2) {
            ans.push_back(x);
...
bool insertTest() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        // if (deq.total_size == ans.size()) {
        //     std::cout << "YYYYESSSSSSS" << std::endl;
        //     std::cout << deq.total_size << " " << ans.size() << std::endl;
        // }
        switch (randnum() % 2) {
            case 0:
